...
class agfs:
    ...
    def startGadgetFuzzer(self, vid=None, pid=None, dclass=None, serial="AutoGadgetFS", manufacturer=None, product=None,
                          samples=100, min=0, max=10):
        ...
        self.gdtzthread = 0
        self.gdtz = threading.Thread(target=self.gdtzfuzz,
                                     args=(vid, pid, dclass, serial, manufacturer, product, samples, min, max,))
        self.gdtz.start()
        ...
        try:
            if self.device:
                if sequence is None and direction is not None:
                    self.searchResults = self.connection.execute(
                        'select distinct RawBinary from "%s" where io="%s"' % (self.dbname, direction)).fetchall()
                    ...
                    count += 1
                    self.searchResults = self.connection.execute(
                        'select distinct RawBinary from "%s" where io="%s" and seq=%d' % (
                            self.dbname, direction, sequence)).fetchone()
    ...
    def searchmsgs(self):
        ...
        self.searchResults = self.connection.execute('select distinct * from "%s" where %s like "%%%s%%"' \
                                                     % (self.dbname, _coldict[self.colSelection],
                                                        self.searcher)).fetchall()
        self.searchdict = {}
    ...
    def setupGadgetFS(self):
        ...
        try:
            ...
            agfsscr.write(
                "echo '%s' | xxd -r -ps > %s/g/functions/hid.usb0/report_desc\n" % (hidreport.decode("utf-8"), basedir))
            ...
            if push2pi == 'y':
                ...
                if rungadget == 'y':
                    ...
                    stdin, stdout, stderr = gogadget.exec_command(
                        f"chmod a+x gadgets/{self.SelectedDevice}.sh;sudo gadgets/{self.SelectedDevice}.sh")
    ...
    def showMessage(self, string, color='green', blink=None):
        ...
        cprint(f"{'*' * (len(string) + 4)}\n{string}\n{'*' * (len(string) + 4)}", color,
               attrs=[] if blink is None else ['blink'])
